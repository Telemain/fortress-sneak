...
public class SpotlightCollide : MonoBehaviour
{
	GameObject Player;
    ...
    void Start()
    {
		...
		target = Quaternion.Euler(Random.Range(90, 130), Random.Range(facing+90, facing+270), 0);
	...
	void FixedUpdate()
    {
		...
		if( true == searching ){
			...
			transform.rotation = Quaternion.Slerp(origin, target, j*0.01f);
			j++;
			
			if( 100 == j ){
				//Debug.Log(j);
				origin = target;
				target = Quaternion.Euler(Random.Range(95, 130), Random.Range(facing+90, facing+270), 0);
				j = 0;
